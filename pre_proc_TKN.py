...
'''
#Classe 1
zeros = pd.DataFrame(np.zeros((c1_train.count(),1)),columns = ['IND_BOM_1_1'])
c1_train = pd.concat([zeros,c1_train.reset_index(drop = True)],axis=1,) #concatena a coluna a direita
#Classe 2
zeros = pd.DataFrame(np.zeros((c2_train.count(),1)),columns = ['IND_BOM_1_2'])
...
'''
#Classe 1
zeros = pd.DataFrame(np.zeros((c1_val.count(),1)),columns = ['IND_BOM_1_1'])
c1_val = pd.concat([zeros,c1_val.reset_index(drop = True)],axis=1,) #concatena a coluna a direita
#Classe 2
zeros = pd.DataFrame(np.zeros((c2_val.count(),1)),columns = ['IND_BOM_1_2'])
...
Teste
'''
zeros = pd.DataFrame(np.zeros((c1_test.count(),1)),columns = ['IND_BOM_1_1'])
c1_test = pd.concat([zeros,c1_test.reset_index(drop = True)],axis=1,) #concatena a coluna a direita
#Classe 2
zeros = pd.DataFrame(np.zeros((c2_test.count(),1)),columns = ['IND_BOM_1_2'])
